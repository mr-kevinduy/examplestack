...
class AbstractRepositoryEloquent extends AbstractRepository
{
    /**
     * Constructor
     */
     ...
     */
    public function getByIndex($columnName = 'id', $value, $columns = ['*'])
    {
        return $this->model->where($columnName, $value)->first($columns);
    }

    /**
     * Get by index or fail.
     *
     * @param  string  $id
     * @param  mixed  $value
     * @param  array|string  $columns
     ...
     */
    public function getByIndexOrFail($columnName = 'id', $value, $columns = ['*'])
    {
        $query = $this->model->where($columnName, $value);

        if (! is_null($result = $query->first($columns))) {
            return $result;
        }
